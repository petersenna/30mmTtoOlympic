...
adapter2_h = 113;
adapter2_external_d = 50; // I added 0.5 for a better fit using my printer
adapter2_base_h = 15;
adapter2_base_d = 70;
adapter2_base_sphere_d = 50;

// Here you may need some experimenting as well
// Thread pitch:
//    https://ars.els-cdn.com/content/image/3-s2.0-B9780750689854000152-gr1.jpg
handle_thread_pitch = 6.5;
// For my printer I adjusted this value to handle_thread_d + 0.35.
// You may need to tune the 0.35 to fit your printer
handle_thread_d = 25.5 + 0.35;

thread_h = 65;

difference() {
    cylinder(d=32, h=adapter2_h, center=true);
    
    // rectangle=0.225 for 1mm thread tooth depth
    // Here you will need to do some experimenting with 
    // rectangle and thread_size.
    translate([0,0,adapter2_h/2-thread_h+1]) metric_thread (diameter=handle_thread_d, pitch=handle_thread_pitch, length=thread_h, square=true, rectangle=0.28, n_starts=1, thread_size=9);

    translate([0,0,((adapter2_h-thread_h)/-2)-12]) cylinder(d=30, h=adapter2_h-thread_h+10, $fn=50, center=true);
    
    // No rounding needed for your handle
    //translate([0,0,-(adapter2_h+50+30)/2]) sphere(50);
}

translate([0,0,0]) difference() {
    union() {
        cylinder(d=adapter2_external_d, h=adapter2_h,center=true);
        translate([0,0,(adapter2_h-adapter2_base_h)/-2]) cylinder(d=adapter2_base_d,h=adapter2_base_h,center=true);
    }
    cylinder(d=32, h=adapter2_h+1,center=true);
    
    // No rounding needed for your handle
    //translate([0,0,-adapter2_h+adapter2_base_sphere_d/2-10]) sphere(50);
}
